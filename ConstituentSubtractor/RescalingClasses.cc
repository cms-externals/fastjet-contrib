...
namespace contrib{
  ...
  BackgroundRescalingYPhiUsingVectorForY::BackgroundRescalingYPhiUsingVectorForY(double v2, double v3, double v4, double psi, std::vector<double> values, std::vector<double> rap_binning, bool interpolate){
    _v2=v2;
    _v3=v3;
    _v4=v4;
    _psi=psi;
  ...
  double BackgroundRescalingYPhiUsingVectorForY::result(const PseudoJet & particle) const {
    ...
    if (_use_rap){
      ...
      if (_interpolate){
        if (rap<(_rap_binning[0]+_rap_binning[1])/2.) rap_term=_values[0];
        else if (rap>=(_rap_binning[nBins-1]+_rap_binning[nBins])/2.) rap_term=_values[nBins-1];
        else{
          ...
          if (rap<rap_bin_center){
            value_low=_values[rap_index-1];
            value_high=_values[rap_index];
            rap_bin_low=(_rap_binning[rap_index-1]+_rap_binning[rap_index])/2.;
            rap_bin_high=rap_bin_center;
          }
          else{
            value_low=_values[rap_index];
            value_high=_values[rap_index+1];
            rap_bin_low=rap_bin_center;
            rap_bin_high=(_rap_binning[rap_index+1]+_rap_binning[rap_index+2])/2.;
          }
          rap_term=value_low+(rap-rap_bin_low)*((value_high-value_low)/(rap_bin_high-rap_bin_low));
        }
      }
      else{
