// $Id: RecursiveSoftDrop.cc 1082 2017-10-10 12:00:13Z gsoyez $
//
...
class RSDHistoryElement{
public:
  RSDHistoryElement(const PseudoJet &jet, const RecursiveSoftDrop* rsd_ptr) :
...
PseudoJet RecursiveSoftDrop::result_fixed_tags(const PseudoJet &jet) const {
  ...
  history.push_back(internal_recursive_softdrop::RSDHistoryElement(ca_jet, this));
  ...
  while ((continue_grooming(n_tagged)) && (active_branches.size())) {
    ...
    // do one step of SD
    RecursionStatus status = recurse_one_step(parent, piece1, piece2, sym, mu2, &R0sqr);
    ...
    if (status==recursion_success){
      ...
      // update the history
      internal_recursive_softdrop::RSDHistoryElement elm1(piece1, this);
      history.push_back(elm1);
      active_branches.push(&(history.back()));
      internal_recursive_softdrop::RSDHistoryElement elm2(piece2, this);
      ...
      if (_dynamical_R0)
        R0sqr = piece1.squared_distance(piece2);
  ...
  while ((continue_grooming(n_depth)) && (active_branches.size())) {
    ...
    while (hist_it!=active_branches.end()){
      ...
      if (result_sd.structure_of<SoftDrop>().has_substructure()){
        ...
        // the next iteration will have to handle 2 new history elements
